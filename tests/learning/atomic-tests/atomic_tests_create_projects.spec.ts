...
import { CreateNewProjectModal } from "../../../src/pages/projects/create_new_project_modal";
...
test.describe("Atomic Tests: Create Project Modal", () => {
  ...
  test("Modal Structure Tests", async ({ page }) => {
    ...
    await test.step("Priority Select Tests", async () => {
      await expect.soft(addProjectModal.prioritySelect).toBeVisible(); // ? nepoužil se await, protože formulář by v této části měl být načtený, takže se čekat už nemusí
      ...
      await expect.soft(addProjectModal.prioritySelect).toHaveValue("34"); // ? kontrola výchozí hodnoty
